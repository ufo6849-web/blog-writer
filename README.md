...
| 블로그 발행 | Blogger API, WordPress REST API, Naver Browser Automation으로 자동 발행 |
...
│   ├── naver_publisher_bot.py Naver 발행
...
# Naver Blog automation (optional)
...
# BananaPro image generation (optional)
BANANAPRO_API_URL=
BANANAPRO_API_KEY=
BANANAPRO_MODEL=banana-pro-image
```
...
### Naver unattended publishing setup

1. Create a dedicated Naver account and a dedicated Chrome profile.
2. Log in to Naver once manually using that profile.
3. Set `NAVER_CHROME_PROFILE_DIR` to that persistent profile directory.
...
5. When calling MCP publishing, use:

```text
blog_publish(platform="naver")
blog_publish(platform="all")
```

The first release publishes with one representative image at the top of the post body. Image priority is `featured_image_path` or `image_path` -> BananaPro -> OpenAI.
...
- Fully unattended publishing uses Playwright with a persistent Chrome profile instead of deprecated official Naver blog write APIs.
- The automation retries transient editor failures up to 3 times.
- If the saved profile is logged out, the run fails fast and alerts Telegram.
- Publishing without the required representative image is treated as a failure.
- `platform="both"` keeps Blogger + WordPress behavior for backward compatibility.
- `platform="all"` publishes to Blogger + WordPress + Naver in one MCP call.

Manual live verification checklist:

```text
1. Log in once with the dedicated Naver Chrome profile.
2. Confirm one image uploads at the top of the post.
3. Confirm title and body text appear correctly.
4. Confirm the final published post URL opens successfully.
```
